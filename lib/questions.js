...
export const calculateScore = (guess, actual) => {
  ...
  if (numGuess > actual) return 0;      // Over = 0 points (Price is Right rule)
  if (numGuess === actual) return 100;  // Exact match = 100 points

  const difference = actual - numGuess;
  const percentOff = difference / Math.abs(actual);

  if (percentOff >= 0.5) {
    // 50%+ under: slope from 60 down to floor of 35
    const score = 60 - ((percentOff - 0.5) * 50);
    return Math.max(35, Math.round(score));
  }

  // 0-50% under: gentle slope from 100 down to 60
  const score = 100 - (percentOff * 80);
